...
* [`src/todoist_clone/`](src/todoist_clone/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/todoist_clone/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `todoist-clone` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/todoist_clone/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns todoist-clone.example
  (:require [todoist-clone.config :as config])
